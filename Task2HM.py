...
def p_vs_p_game(candies_sum: int) -> bool:
    ...
    while candies_sum > 0:
        ...
        if (candies_sum > 0 and correct_first):
...
# первый игрок победит, если возьмет 19 конфет в начале, а потом будет добивать до 29 любую пару
# взяток. И в итоге он в любом случае не будет брать последним
